...
class VoiceAgentClient(AsyncClient):
    ...
    def _calculate_ttft(self) -> None:
        """Calculate the time to first text.

        To calculate the TTFT, we take the first word partial and calculate
        the time difference between the time the word was received and the
        time reported in the payload from the STT engine.
        """
        # Skip if no fragments
        if not self._speech_fragments:
            return

        # Find first fragment that is a 'word' and not is_final
        first_word = next(
            (f for f in self._speech_fragments if f._type == "word"),
            None,
        )
        ...
        if (
            not first_word
            or first_word._type != "word"
            or first_word.end_time == self._last_ttft_time
        ):
            return

        # Debug
        logger.debug(
            f"TTFT `{first_word.content}` at {first_word.end_time}; total time: {self._total_time}"
        )

        # Calculate the time difference (convert to ms)
        self._last_ttft = round((self._total_time - first_word.end_time) * 1000)
        self._last_ttft_time = first_word.end_time
